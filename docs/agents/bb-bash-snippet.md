...
Use `bbb` for Bitbucket Cloud operations. This instruction is self-contained; the optional `bbb` skill provides the same workflow for lazy or explicit activation (`/bbb` in Claude Code, `$bbb` in Codex). Run `bbb help <command>` for current syntax and `bbb help` for the command list. The installed CLI is authoritative.
